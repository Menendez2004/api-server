...
export class TokenService {
  ...
  async createToken(
    ...
    try {
      return await this.prismaService.verificationTokens.create({
        data: token,
      });
    } catch (error) {
      if (error instanceof Prisma.PrismaClientKnownRequestError) {
        throw new InternalServerErrorException(
          `Prisma error code: ${error.code}, Message: ${error.message}`,
        );
      }
      throw new InternalServerErrorException(
  ...
  async findAuthToken(token: string): Promise<VerificationTokens> {
    const authToken = await this.prismaService.verificationTokens.findFirst({
      where: { token },
    });
    if (!authToken) {
