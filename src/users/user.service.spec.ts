...
describe('UsersService', () => {
    ...
    describe('create', () => {
        it('this should create a new user', async () => {
            const userMock = { id: '0fe3dbd4-aee0-47db-ac4e-56e2e3382a15', email: 'johndoe@example.com' } as any;
            ...
            expect(prismaService.users.create).toHaveBeenCalledWith({ data: MocksUserService.userCreate });
            expect(result).toEqual(userMock);
        });

        it('this should return an error if the user creation fails  ', async () => {
            jest.spyOn(prismaService.users, 'create').mockRejectedValue(new Error('Error creating user'));
        });
    });
    ...
    describe('findByEmail', () => {
        it('should return a user by email', async () => {
            const mockUser = { id: '0fe3dbd4-aee0-47db-ac4e-56e2e3382a15', email: 'johndoe@example.com' } as any;
            jest.spyOn(prismaService.users, 'findUnique').mockResolvedValue(mockUser);
            const result = await userService.findByEmail(MocksUserService.userFindByEmail.email);
            expect(prismaService.users.findUnique).toHaveBeenCalledWith({
        ...
        it('should throw an error if an exception occurs', async () => {
            jest.spyOn(prismaService.users, 'findUnique').mockRejectedValue(new Error('Database error'));
            await expect(userService.findByEmail('error@example.com')).rejects.toThrow('Database error');
        });
    });
    describe('hashPass', () => {
        ...
        it('should throw InternalServerErrorException if hashing fails', async () => {
            jest.spyOn(require('bcrypt'), 'hash').mockRejectedValue(new Error('Hashing error'));
    ...
    describe('getUserRole', () => {
        it('should return the user role for a valid user ID', async () => {
            ...
            jest.spyOn(prismaService.userRoles, 'findUnique').mockResolvedValue(userRoleMock);
        ...
        it('should throw NotFoundException if user role is not found', async () => {
            ...
            await expect(userService.getUserRole(userMock.id)).rejects.toThrow(NotFoundException);
        });
    ...
    describe('resetPass', () => {
        it('should reset the user password successfully', async () => {
            ...
            jest.spyOn(tokenService, 'findAuthToken').mockResolvedValue(tokenMock as any);
            jest.spyOn(prismaService.users, 'update').mockResolvedValue(true as any);
            jest.spyOn(prismaService.verificationTokens, 'update').mockResolvedValue(true as any);
            const result = await userService.resetPass(verificationToken, newPassword);
            expect(tokenService.findAuthToken).toHaveBeenCalledWith(verificationToken);
        ...
        it('should throw InternalServerErrorException if password reset fails', async () => {
            jest.spyOn(tokenService, 'findAuthToken').mockRejectedValue(new Error('Token error'));
            await expect(
